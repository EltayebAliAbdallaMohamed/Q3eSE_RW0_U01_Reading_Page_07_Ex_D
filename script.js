...
async function loadData() {
  try {
    const response = await fetch('Q3eSE_RW0_U01_Reading_Page_07_EX_D.json');
    ...
    alert('Failed to load lesson data. Please check that Q3eSE_RW0_U04_Reading.json exists.');
  }
}
...
// ========== TTS PLAYER BUTTON (Main Play Button) ==========
...
// ========== TTS STOP BUTTON (Stop Main Play) ==========
...
function nextSentence() {
  ...
  if (navPos < navIndices.length - 1) {
    ...
    if (autoPlayCheckbox.checked) {
      // Auto-play using TTS instead of audio element
      setTimeout(function () {
        playTTSBtn.click();
      }, 500);
    }
